...
allT: compileT executeT

cleanT: 
	rm -f $(TEMPPATH)/logTest.txt  $(TEMPPATH)/programTest.hex  $(TEMPPATH)/TESTTest.dis  $(TEMPPATH)/TESTTest.exe
	
compileT:
	$(GCC_PREFIX)-gcc $(ABI) -lgcc -T$(LINK) -o  $(TEMPPATH)/TESTTest.exe $(CODEFOLDER)/Test.s -nostartfiles -lm
	$(GCC_PREFIX)-objcopy -O verilog  $(TEMPPATH)/TESTTest.exe  $(TEMPPATH)/programTest.hex
	$(GCC_PREFIX)-objdump -S  $(TEMPPATH)/TESTTest.exe >  $(TEMPPATH)/TESTTest.dis
	
executeT:
	-whisper -x $(TEMPPATH)/programTest.hex -s 0x80000000 --tohost 0xd0580000 -f  $(TEMPPATH)/logTest.txt --configfile ./veer/whisper.json
	python3 python/print_log_array.py -1 Test
